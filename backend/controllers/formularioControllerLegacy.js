// backend/controllers/formularioControllerLegacy.js
...
if (codigoUp === "ANEXO_02") {
  ...
  signers02 = { postulanteNombre, adminNombre };
    ...
    if (codigoUp === "ANEXO_01") {
      ...
      renderAnexo09Pdf(doc, anexo, vivienda, signers09, historial);
...
async function darConformidadPermisionario09(req, res) {
  try {
    ...
    d.conformidadPermisionario = {
      ...
      observacion:
        d.observacionesPermisionario ||
        "Conformidad del permisionario sobre el ANEXO 09.",
    };
...
// ─────────────────────────────
// ANEXO_09: CIERRE ADMIN_GENERAL (con o sin novedades)
async function cerrarAnexo09AdminGeneral(req, res) {
  try {
    ...
    const tieneConforme =
      datos.conformidadPermisionario && datos.conformidadPermisionario.ok;
    ...
    if (!tieneConforme) {
      ...
      // cierre sin novedades — la observación es opcional
      if (obsCrit) {
        datos.observacionCritica = obsCrit;
      }
      anexo.estadoInstitucional = "CERRADO_SIN_NOVEDADES";
    }
    ...
    datos.conformidadAdminGeneral = {
      ...
      usuario: user._id,
      observacion:
        datos.observacionCritica ||
        obsCrit ||
        "Cierre ADMIN_GENERAL del ANEXO 09.",
    };
    ...
    if (!viviendaId || !isObjectId(viviendaId)) {
      return res.status(403).json({ message: "Recurso no disponible" });
    }
...
// ─────────────────────────────
// ANEXO_11 – Gestión ADMIN_GENERAL / ADMIN
// POST /api/formularios/:id/gestion-admin-11
// body: { accion: "CERRAR" | "DEVOLVER_A_INSPECTOR", observacionesAdminGeneral?: string }
