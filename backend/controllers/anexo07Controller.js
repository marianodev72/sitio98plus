// controllers/anexo07Controller.js
 ...
 * - ADMIN_GENERAL
 */
async function crearAnexo07(req, res) {
  try {
    ...
    const nuevoAnexo07 = new FormSubmission({
      tipo: 'ANEXO_07',
    ...
    await nuevoAnexo07.save();

    return res.status(201).json({
      message: 'ANEXO 07 creado correctamente',
      anexo07Id: nuevoAnexo07._id,
    });
  } catch (err) {
    console.error('Error al crear ANEXO 07:', err);
    return res.status(500).json({ error: 'Error interno al crear ANEXO 07' });
  }
}
...
async function obtenerPorId(req, res) {
  try {
    ...
    if (!anexo || anexo.tipo !== 'ANEXO_07') {
      return res.status(404).json({ error: 'ANEXO 07 no encontrado' });
    }

    return res.json(anexo);
  } catch (err) {
    console.error('Error al obtener ANEXO 07:', err);
...
async function listar(req, res) {
  try {
    ...
    console.error('Error al listar ANEXO 07:', err);
    return res.status(500).json({ error: 'Error interno' });
  }
}
...
async function cambiarEstado(req, res) {
  try {
    ...
    console.error('Error al cambiar estado ANEXO 07:', err);
    return res.status(500).json({ error: 'Error interno' });
  }
}
