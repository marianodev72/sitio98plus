    ...
    const fetchGestiones = async () => {
      try {
        ...
        if (resp.status === 401) {
          const message =
            (data && data.message) ||
            "No estás autenticado. Volvé a iniciar sesión.";
          throw new Error(message);
        }

        // 404: tratamos como lista vacía, sin romper nada
        if (resp.status === 404) {
          setGestiones([]);
          return;
        }
        ...
        if (!resp.ok || (data && data.ok === false)) {
          const message =
            (data && data.message) ||
            "No se pudieron obtener tus gestiones.";
          throw new Error(message);
        }

        const lista =
          (data && (data.gestiones || data.anexo11 || data.items || data.data)) ||
          [];

        setGestiones(Array.isArray(lista) ? lista : []);
      } catch (err) {
        console.error("[GESTIONES MIS] Error al cargar gestiones:", err);
  ...
  return (
    <div className="min-h-screen bg-[#050816] text-white">
      <div className="max-w-6xl mx-auto px-4 py-10">
        {/* Volver al panel de gestiones */}
        <button
          className="text-sm text-sky-400 hover:text-sky-300 mb-4"
          onClick={() => navigate("/permisionario/gestiones")}
        >
          ← Volver a Gestiones
        </button>
        ...
        {/* Mensajes de estado */}
        {loading && (
          <div className="text-slate-300 text-sm bg-[#0b1020] border border-slate-700/60 rounded-xl px-4 py-3 mb-4">
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {gestiones.map((g) => {
                    ...
                    const tipo =
                      g.tipoSolicitud || g.tipo || g.solicita || "—";
                    const estado =
                      g.estado || g.estadoActual || "ENVIADO";
                    const fecha =
                      g.creadoEn || g.createdAt || g.fechaCreacion || null;
                    const detalle =
                      g.detallePedido || g.detalle || g.descripcion || "";
