...
export default function MisAnexos() {
  ...
  async function cargar() {
    ...
    try {
      ...
      console.error("[MIS ANEXOS PERM] Error", e);
  ...
  async function descargarPdf(id: string, cod: string) {
    ...
    try {
      ...
      console.error("[MIS ANEXOS PERM] Error PDF", e);
  ...
  // Permisionario e Inspector pueden consultar
  if (role !== "PERMISIONARIO" && role !== "INSPECTOR") {
    return (
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <section style={{ marginBottom: 12, padding: 12, border: "1px solid #ddd" }}>
        ...
        <button onClick={cargar} disabled={loading} style={{ marginLeft: 8 }}>
              ...
              {items.map((an) => (
                <tr key={an._id}>
                  ...
                  <td style={{ whiteSpace: "nowrap" }}>
                    ...
                    <button onClick={() => descargarPdf(an._id, up(an.codigo))}>PDF</button>
                  </td>
                </tr>
              ))}
            </tbody>
