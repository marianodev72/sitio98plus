// src/pages/AdministracionAnexo4Listado.jsx
import React, { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";

const API_BASE_URL = "http://127.0.0.1:3000";

const AdministracionAnexo4Listado = () => {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        const resp = await fetch(`${API_BASE_URL}/api/anexo4/admin`, {
  ...
  if (user && !(user.role === "ADMIN" || user.role === "ADMINISTRACION")) {
    return (
  ...
  return (
    <div className="min-h-screen bg-[#050816] text-white">
      <div className="max-w-6xl mx-auto px-4 py-10">
        <h1 className="text-2xl font-bold mb-2">Anexo 4 – Administración</h1>
        <p className="text-slate-400 mb-6">
          Visualización informativa de los formularios Anexo 4.
        </p>
              ...
              <tbody>
                {gestiones.map((g) => (
                  <tr
                    ...
                    onClick={() =>
                      navigate(`/administracion/gestiones/anexo4/${g.id}`)
                    }
                  >
...
export default AdministracionAnexo4Listado;
