// src/pages/AdministracionAnexo4Detalle.jsx
import React, { useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";

const API_BASE_URL = "http://127.0.0.1:3000";

const AdministracionAnexo4Detalle = () => {
  const { id } = useParams();
  ...
  return (
    <div className="min-h-screen bg-[#050816] text-white">
      <div className="max-w-4xl mx-auto px-4 py-10">
        <button
          onClick={() => navigate("/administracion/gestiones/anexo4")}
          ...
          <div className="bg-[#0b1020] border border-slate-700 rounded-2xl p-6 space-y-4">
            ...
            {anexo.jefeBarrio?.usuario && (
              <div className="border-t border-slate-700 pt-4 text-sm">
                <h2 className="font-semibold text-slate-300 mb-1">
                  Recepción Jefe de Barrio
                </h2>
                <p>Jefe de Barrio: {anexo.jefeBarrio.nombreCompleto}</p>
                <p className="text-xs text-slate-400">
                  {new Date(
                    anexo.jefeBarrio.fechaRecepcion
...
export default AdministracionAnexo4Detalle;
